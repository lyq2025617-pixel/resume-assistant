...
输入文本是一份简历，可能包含以下信息：
- 个人信息（姓名、电话、邮箱、籍贯/城市、政治面貌）
- 教育背景（学校、专业、类型、入学时间、排名、GPA、均分）
- 技能标签（按核心能力、工具能力、技术能力、产品能力分类）
...
4. 保持语言与原文一致，不做改写
5. skills 数组中的每个条目必须带有分类前缀，例如：
   "核心能力：xxx"、"工具能力：xxx"、"技术能力：xxx"、"产品能力：xxx"
   如果原文中没有明确分类，请根据内容自动归类
6. 个人信息必须尽量提取：姓名、电话、邮箱（这三个最关键）
7. 严格按照以下 JSON 格式输出：
...
def _extract_text(msg) -> str:
    """兼容 Claude 和 Qwen 等非标准模型的响应格式。
    Qwen 有 thinking block（思考过程）+ text block（正式回答），
    我们只需要 text block。"""
    parts = []
...
def parse_to_resume_data(
    ...
    msg = client.messages.create(
        ...
        temperature=0.3,
        thinking={"type": "disabled"},  # 关闭 thinking 加速响应
    )
